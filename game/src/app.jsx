...
import { calculateSelectionTotal, getVisibleMemoryIds, variantDisplayLabel } from "./engine/game-engine";

const storageKey = "千字村公开试玩版-v1-存档";
const legacyStorageKey = "千字村十五回合详细证词原型十存档";

function readStoredGame() {
  try {
    return JSON.parse(localStorage.getItem(storageKey) || localStorage.getItem(legacyStorageKey) || "null");
  } catch (_) {
    return null;
  }
}
...
function calculateTotal(memoryData, selections) {
  return calculateSelectionTotal(memoryData, selections);
}

function getVisibleIds(memoryData, roundIndex) {
  return getVisibleMemoryIds(memoryData.initialCards, window.roundStories.map(function (story) { return story.id; }), roundIndex);
}

function labelForVariant(variantId) {
  return variantDisplayLabel(variantId, window.variantLabels);
}
...
function TitleScreen(props) {
  return (
    <section className="screen title-screen" data-screen-label="标题页">
      <img className="full-bleed-image" src="assets/imagegen/01-入口标题图片原型.webp" alt="" />
      ...
      <div className="title-content">
        ...
        <div className="scope-note">
          ...
          <span>公开试玩包含全部 15 回合，完整呈现记忆取舍、红线托付、延迟后果与最终传碑。</span>
        </div>
      </div>
    </section>
  );
...
function DutyScreen(props) {
  ...
  return (
    <section className="screen duty-screen" data-screen-label="守字人的职责">
      <img className="full-bleed-image" src="assets/imagegen/03-碑片结构纯背景.webp" alt="" />
...
const prologuePages = [
  {
    kicker: "序章·守字人的清晨",
    title: "上一任守字人已经离去",
    image: "assets/imagegen/02-序章职责图片原型.webp",
  ...
  },
  {
    kicker: "序章·碑片更替",
    title: "整座碑不动，旧文字仍会离开",
    image: "assets/imagegen/03-碑片结构纯背景.webp",
  ...
  return (
    <section className="screen old-stele-screen" data-screen-label="阅读旧碑">
      ...
      <div className="old-stele-backdrop">
        <img src="assets/imagegen/04-整碑阅读纯背景.webp" alt="" />
      </div>
    ...
    <section className="screen overview-screen" data-screen-label={props.step === 0 ? "玩法概览首图" : props.step === 1 ? "碑片更新教学" : "玩法四步教学"}>
      ...
      <div className="overview-body">
        ...
        <figure className="overview-figure">
          <img
            src={props.step === 0 ? "assets/reference/核心玩法全景.webp" : props.step === 1 ? "assets/imagegen/03-碑片结构纯背景.webp" : "assets/imagegen/05-超限与红线图片原型.webp"}
...
function RoundOpeningScreen(props) {
  ...
  const narrative = window.roundNarrativeV10[props.story.id];
...
function EventScreen(props) {
  ...
  const narrative = window.roundNarrativeV10[story.id];
    ...
    <section className="screen event-screen" data-screen-label={"回合" + story.round + "事件"}>
      ...
      <div className="event-grid">
        ...
        <aside className="testimony-panel">
          ...
          <p className="testimony-expanded-note">详细证词已展开。请向下滚动，读完每个人亲眼看见的事，以及他们担心村庄失去什么。</p>
          {story.round === 1 ? (
          ...
          <div className="testimony-list">
            {story.testimony.map(function (item) {
              const expanded = !collapsedSpeakers.includes(item.name);
              const detail = narrative.details && narrative.details[item.name];
              return (
                <article className={"testimony-card " + (expanded ? "expanded" : "collapsed")} key={item.name}>
                  <span className="speaker-seal">{item.name.slice(0, 1)}</span>
                  <div>
                    <h2>{item.name}<small>{item.role}</small></h2>
                    <span className="testimony-section-label">希望碑上留下</span>
                    <p className="testimony-claim">{item.text}</p>
                    {expanded ? (
                      <div className="testimony-detail">
                        {detail ? (
                          <React.Fragment>
                            <span className="testimony-section-label">亲眼看见</span>
                            <p className="testimony-more">{detail.witnessed}</p>
                            <span className="testimony-section-label testimony-fear-label">担心失去</span>
                            <p className="testimony-more testimony-fear">{detail.fears}</p>
                          </React.Fragment>
                        ) : (
                          <React.Fragment>
                            <span className="testimony-section-label">亲眼看见，以及担心失去</span>
                            <p className="testimony-more">{narrative.more[item.name]}</p>
                          </React.Fragment>
                        )}
                      </div>
                    ) : null}
                    <button className="listen-more" type="button" aria-expanded={expanded} onClick={function () {
                      setCollapsedSpeakers(function (current) {
                        return current.includes(item.name)
                          ? current.filter(function (name) { return name !== item.name; })
                          : current.concat(item.name);
                      });
                    }}>{expanded ? "收起详细证词" : "展开详细证词"}</button>
                  </div>
                </article>
              );
            })}
...
function ReactionScreen(props) {
  ...
  return (
    <section className="screen reaction-screen" data-screen-label={"回合" + story.round + "封存反馈"}>
      <img className="full-bleed-image" src="assets/imagegen/06-封存因果图片原型.webp" alt="" />
      <div className="dark-wash"></div>
...
function VillageInterludeScreen(props) {
  const interlude = window.interludesV10[props.afterRound];
  ...
  return (
    <section className="screen village-interlude-screen" data-screen-label={"第" + props.afterRound + "回合后的村庄"}>
      <img className="full-bleed-image" src={props.afterRound === 5 ? "assets/scenes/scene-06-council.webp" : "assets/scenes/scene-11-refugees.webp"} alt="" />
...
function EpilogueWalkScreen(props) {
  const locations = window.createEpilogueLocationsV10(props.selections);
  const [placeIndex, setPlaceIndex] = useState(0);
  ...
  return (
    <section className="screen epilogue-walk-screen" data-screen-label="洪水后的千字村">
      <img className="epilogue-village-image" src="assets/scene-16-post-flood-village.webp" alt="洪水退去后正在修复的千字村" />
  ...
  return (
    <section className="screen summary-screen" data-screen-label="村志附录">
      <div className="summary-hero">
        <img src="assets/scene-16-post-flood-village.webp" alt="" />
        <div>
...
function App() {
  ...
  useEffect(function () {
    fetch("data/memory-cards.json")
      ...
      .catch(function () {
        document.getElementById("root").innerHTML = "<p class='fatal-message'>记忆卡读取失败。请重新载入游戏。</p>";
      });
  }, []);
  ...
  return <div className="fatal-message">游戏状态无法识别，请重新开始。</div>;
}
