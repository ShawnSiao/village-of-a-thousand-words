...
const storageKey = "千字村十五回合交互原型八存档";
    ...
    <section className="screen title-screen" data-screen-label="标题页">
      ...
      <div className="title-content">
        ...
        <p className="title-intro">
          一座村庄共享着一份只能容纳一千字的公共记忆。每一次新生、灾难、友情与流言，都会迫使另一段往事被压缩、改写或遗忘。你将接任守字人，决定哪一种说法被留在碑上；村庄则按照留下的文字继续生活。
        </p>
      ...
      <div className="title-quote">「你不能替村庄选择未来。你只能决定，它带着哪些记忆走向未来。」</div>
    </section>
  );
}
...
const prologuePages = [
  {
    ...
    roleTitle: "你的身份：守字人",
    roleBody: "从这一刻起，由你决定碑上记录什么，也决定哪些旧事被压缩或遗忘。",
    ...
    <section className="screen reaction-screen" data-screen-label={"回合" + story.round + "封存反馈"}>
      ...
      <article className="reaction-sheet">
        ...
        <button className="paper-button" type="button" onClick={props.onContinue}>
          {story.round === 15 ? "查看三十年后的千字村" : story.round === 5 || story.round === 10 ? "进入下一阶段" : "让岁月继续"}
        </button>
      </article>
    </section>
  );
...
function SummaryScreen(props) {
  ...
  return (
    <section className="screen summary-screen" data-screen-label="三十年后的千字村">
      <div className="summary-hero">
        <img src="assets/scenes/scene-15-final-flood.png" alt="" />
        <div>
          <span className="eyebrow">第三十年·传碑之秋</span>
          <h1>村庄已经成为那一千字留下的样子</h1>
          <p>洪水不是一项总分。下面每一项行动，都能回到某段仍在的碑文，也能回到某段已经离开的过去。</p>
        </div>
      </div>
