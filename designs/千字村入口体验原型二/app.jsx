...
function TitleScreen(props) {
  return (
    <section className="screen title-screen" data-screen-label="标题页">
      <img className="full-bleed-image" src="assets/imagegen/01-标题页图片原型.png" alt="" />
      ...
      <div className="title-content">
        ...
        <p className="title-intro">
          一座村庄共享着一份只能容纳一千字的公共记忆。每一次新生、灾难、友情与流言，都会迫使另一段往事被压缩、改写或遗忘。
        </p>
        ...
        <div className="scope-note">
          <strong>完整最小可行版本：15 回合</strong>
          <span>本次可试玩前 5 回合，约 8～12 分钟，用于验证规则理解、记忆取舍和因果回溯。</span>
        </div>
      </div>
...
function DutyScreen(props) {
  const duties = [
    ...
    ["二", "决定公共说法", "从完整记录与两种压缩写法中，选择刻入千字碑的版本。"],
  ...
  return (
    <section className="screen duty-screen" data-screen-label="守字人的职责">
      ...
      <article className="duty-sheet">
        ...
        <h1>守字人的职责</h1>
        <p className="lead">守字人没有直接命令村民的权力。你的权力更间接，也更难推卸：决定哪一种过去能够继续成为全村的共同事实。</p>
        ...
        <div className="duty-boundary">
          <strong>你不会逐字写一千字。</strong>
          <span>所有事件、记忆和压缩版本均已由设计者写定。你的选择是保留哪一种意义。</span>
        </div>
...
const prologuePages = [
  {
    kicker: "序章·守字人的清晨",
    title: "上一任守字人已经离去",
    body: "雨停后的清晨，祠堂里只剩一张空椅、未干的墨和一把旧刻刀。那根系在旧守字人手腕上的红线，另一端仍缠在千字碑上。",
    note: "从今天起，你是新的守字人。"
  },
  {
    kicker: "序章·共同记忆",
    title: "碑上文字不会让往事立刻消失",
    body: "亲历者仍会记得碑外的细节，也可能反驳碑文。可当他们老去、离开，年轻人只会知道石面上留下的版本。多年以后，碑文将成为全村共同记得的事实。",
    note: "你改变的不是昨天，而是昨天还能怎样抵达明天。"
  },
  {
    kicker: "序章·四条规则",
    title: "刻下第一句话以前",
...
function PrologueScreen(props) {
  ...
  return (
    <section className="screen prologue-screen" data-screen-label={"序章第" + (props.step + 1) + "页"}>
      <img className="full-bleed-image" src="assets/imagegen/02-序章图片原型.png" alt="" />
  ...
  return (
    <section className="screen old-stele-screen" data-screen-label="阅读旧碑">
      ...
      <div className="old-stele-backdrop">
        <img src="assets/imagegen/03-旧碑阅读图片原型.png" alt="" />
      </div>
...
function OverviewScreen(props) {
  ...
  return (
    <section className="screen overview-screen" data-screen-label={props.step === 0 ? "玩法概览首图" : "玩法四步教学"}>
      <AppHeader onHome={props.onHome} onReset={props.onReset} />
      <div className="overview-body">
        <div className="overview-copy">
          <span className="eyebrow">{props.step === 0 ? "玩法概览" : "守字人的一年"}</span>
          <h1>{props.step === 0 ? "每一次记录，都会挤压另一段过去" : "四步完成一次公共记忆"}</h1>
          <p>{props.step === 0
            ? "这张完整首图展示一回合的核心结构：左侧是事件与证词，中间是千字碑和记述版本，右侧是容量与封存。它现在作为正式教学画面出现，不再只是低透明度背景。"
            : "界面会逐步提示当前动作。第一回合不会强迫你修改旧记忆，先学会比较三种写法；从第二回合起，字数压力会开始逼近。"
          }</p>
          {props.step === 1 ? (
          ...
          <button className="primary-button" type="button" onClick={props.onNext}>
            {props.step === 0 ? "查看四步操作" : "进入第一回合"}
          </button>
        </div>
        <figure className="overview-figure">
          <img
            src={props.step === 0 ? "assets/imagegen/05-既有玩法首图.png" : "assets/imagegen/04-玩法教学图片原型.png"}
            alt={props.step === 0 ? "千字村完整玩法首图" : "四步玩法教学图片"}
          />
          <figcaption>{props.step === 0 ? "完整玩法首图" : "一：证词　　二：版本　　三：容量　　四：封存"}</figcaption>
        </figure>
      </div>
    </section>
  );
...
function MemoryRail(props) {
  return (
    <aside className="memory-rail">
      <div className="panel-heading"><span>碑上记忆</span><strong>选择一段查看或整理</strong></div>
      <div className="memory-rail-scroll">
        {props.visibleIds.map(function (id) {
          ...
          return (
            <button
              className={"memory-rail-item " + (props.selectedId === id ? "selected " : "") + (isCurrent ? "current " : "") + (deleted ? "deleted" : "")}
              type="button"
              key={id}
              onClick={function () { props.onSelect(id); }}
            >
    ...
    <aside className="variant-panel">
      ...
      <div className="variant-action">
        {props.total > props.capacity ? (
          <p className="overflow-message">石面已满。还需从左侧选择旧记忆，缩短 {props.total - props.capacity} 字。</p>
        ) : props.currentReady ? (
    ...
    <section className="screen editor-screen" data-screen-label={"回合" + props.story.round + "碑文编辑"}>
      ...
      <div className="editor-grid">
        <MemoryRail
          ...
          onUndo={props.onUndo}
        />
        <StelePreview card={card} variant={variant} isCurrent={isCurrent} isDraft={isCurrent && !variantId} isDeleted={isDeleted} />
        <VariantPanel
          card={card}
...
function ReactionScreen(props) {
  ...
  return (
    <section className="screen reaction-screen" data-screen-label={"回合" + story.round + "封存反馈"}>
      <img className="full-bleed-image" src={story.image} alt="" />
      <div className="dark-wash"></div>
      <article className="reaction-sheet">
        <div className="reaction-heading"><span>封</span><div><small>{story.year}</small><h1>本年已经写定</h1></div></div>
        ...
        <section className="echo-card">
          ...
          <p>{props.echo.now}</p>
          <p>{props.echo.later}</p>
          <small>{props.echo.source}</small>
        </section>
  ...
  if (game.view === "overview") {
    return (
      <OverviewScreen
        ...
        onNext={function () {
          if (game.overviewStep === 0) {
            setGame(function (current) { return Object.assign({}, current, { overviewStep: 1 }); });
          } else {
